...
<script type="text/javascript">
	...
	function fn_memberJoin() {
		...
		for(var i=0;i<file.length;i++) {
			if (file[i].files[0] != undefined) {
				if(!checkFileExt(file[i].files[0].name, "png")) {
					alert("직인파일 업로드시 이미지파일은 PNG포맷만 가능합니다.");
					return;
				}
	    ...
	    formData.append("funnel",<%= request.getParameter("funnel")%>);
	    formData.append("funnelYear",<%= request.getParameter("funnelYear")%>);
				...
				<div id="terms_inner_step02">
					<ul id="join_info">
                        ...
                        <li class="info manager_info column1">
                            <div class="info_tit NanumGothic">기업 정보</div>
                            <dl class="input_bx">
						...
						<li class="tit NanumGothic">
							<span>※ 직인 이미지는 배경이 투명한 이미지(PNG)파일로 등록 부탁드리며, <br>이미지 사이즈(Pixel)는 100x100 사이즈로 작업해주시기 바랍니다.</span>
						</li>
