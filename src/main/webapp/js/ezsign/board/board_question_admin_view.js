		...
		success:function(res) {		
			...
			if (res.success) {
				...
				for(var idx = 0; idx < model.bbsList.length; idx++){		
					...
					if(model.detail.bbsId == row.bbsId){
				...
				if(userInfo.loginType >= model.detail['userType']){
					btnGroupHtml += "<a class='btn_type' onClick='doConfirm()'><span class='icon Material_icons'>check_circle</span>"+ model.detail['statusName'] +"</a>";
				}
				...
				commentListHtml +="<div class='area head_group'><div class='company'>"+getCookie("bizName")+"</div><div class='userName'>"+getCookie("loginName")+"</div></div>";
...
function goList() {
	...
	$.ajax({
		url: rootPath + "html/board/board_question_admin_list.html",
...
function doConfirm() {
    ...
    if (model.detail['statusCode'] == '0') {
    	statusCode = '1';
    	if (model.comment.length == 0) {
    		confirmMsg = '댓글을 남기지 않고 ' + statusName[statusCode] + ' 상태로 변경 하시겠습니까?';
    	} else {
    		confirmMsg = statusName[statusCode] + ' 상태로 변경 하시겠습니까?';
    	}
    } else if (model.detail['statusCode'] == '1') {
    	statusCode = '0';
    	confirmMsg = statusName[statusCode] + ' 상태로 변경 하시겠습니까?';
    }

    jConfirm(confirmMsg,'',function(cfrm){
    	...
    	if(cfrm){
    		...
    		$.ajax({
    			...
    			success:function(res) {		
    				...
    				if (res.success) {
    					jAlert(statusName[statusCode] + ' 상태로 변경 되었습니다.','',function(){
    						getDetail();
    					});    					
    				}else{
    					$.unblockUI();
