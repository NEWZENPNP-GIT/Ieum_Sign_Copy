...
function getList() {
	...
	formData.append('contentType', model.search.contentType);
	formData.append('sortName', "A.EXPIRE_DATE");
			...
			if (res.success) {
	    	    ...
	    	    if(model.list != null && model.list.length > 0){	    	    	
	    	    	...
	    	    	for(var idx = 0 ; idx < model.list.length ; idx++){
	    	    		...
	    	    		if(isNotNull(row.expireDate)){ 
	    	    			if(moment(row.expireDate,"YYYYMMDD").format("YYYYMMDD") < new Date().format("yyyyMMdd")){
	    	    				noticeListHtml +="<td class='value'><span style='color:red;'>[만료]"+moment(row.expireDate,"YYYYMMDD").format('YYYY-MM-DD')+"</span></td>";
	    	    			} else {
	    	    				noticeListHtml +="<td class='value'>"+moment(row.expireDate,"YYYYMMDD").format('YYYY-MM-DD')+"</td>";
	    	    			}
				    	}else{
				    		noticeListHtml +="<td class='value'></td>";
				    	}	
...
function fncBoardWrite(){
	...
	$.ajax({
		url: rootPath + "html/board/board_main_notice_write.html",
