...
$(document).ready(function() {
	...
	if(isNotNull(getCookie("paramBbsId"))){
		model.write.bbsId = getCookie("paramBbsId");
...
function initCode() {
		...
		success:function(res) {		
			...
			if (res.success) {
				...
				if (isNull(model.write.bbsId)) {
					...
					$.each(res['data'], function(i, row) {							
						if (row.refCode == model.write.bbsId) {
							model.write.commCode = row.commCode;
						}
					});
				}
    ...
    url = rootPath + 'code/getCodeList.do';
		...
		success:function(res) {		
			...
			if (res.success) {				
			    ...
			    if(boardWorkCodeList != null && boardWorkCodeList.length > 0){
					...
					$("#model_write_workCode").html("<option value=''>전체</option>");
				}
				...
				jAlert('등록 되었습니다.','',function(){
					...
					$.ajax({
						...
						success: function(result) {			
							setCookie("paramBbsId", model.write.bbsId);
