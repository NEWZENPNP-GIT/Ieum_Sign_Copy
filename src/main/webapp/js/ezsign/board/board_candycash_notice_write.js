...
$(document).ready(function() {
	...
	//if( (userInfo.loginType > 8 && userInfo.loginType >= model.write['userType']) || userInfo.userId == model.write['userId']){
	if( (userInfo.loginType > 8) || userInfo.userId == model.write['userId']){
		$("#btnGroupDiv").html("<a class='btn_type Material_icons' onClick='doWrite()'><span class='icon Material_icons'>check_circle</span>작성완료</a>");
	}
		...
		success:function(res) {		
			...
			if (res.success) {
				jAlert('등록 되었습니다.','',function(){
					...
					$.ajax({
						url: rootPath + "html/board/board_candycash_notice_list.html",
