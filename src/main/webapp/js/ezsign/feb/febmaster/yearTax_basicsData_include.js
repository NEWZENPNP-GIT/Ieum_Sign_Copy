...
function getCompanyWorkplaceInfo() {
	...
	logD("# include 계약ID : " + yearContractId );
			...
			if (res['success']) {					
				...
				if(edinfoList != null && edinfoList.length > 0){
					...
					for(var i = 0 ; i < edinfoList.length ; i++){
						if(edinfoList[i]['진행상태코드'] == "1"){
							statName = "진행중";
						}else if(edinfoList[i]['진행상태코드'] == "2"){
							statName = "근로자 마감";
						}else if(edinfoList[i]['진행상태코드'] == "3"){
							statName = "중간관리자 마감";
						}else if(edinfoList[i]['진행상태코드'] == "4"){
							statName = "관리자 마감";
						}else if(edinfoList[i]['진행상태코드'] == "5"){
							statName = "최종확정";
						}						
						...
						empListHtml += "<div class='state code-"+edinfoList[i]['진행상태코드']+"'><p style='cursor:pointer'>"+statName+"</p></div>";
						empListHtml += "</li>";
					}
