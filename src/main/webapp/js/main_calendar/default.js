...
(function(window, Calendar) {
  ...
  cal = new Calendar('#calendar', {
    ...
    template: {
      ...
      allday: function(schedule) { return getTimeTemplate(schedule, true); },
      time: function(schedule) { return getTimeTemplate(schedule, false); }
    }
  });
  ...
  cal.on({
    ...
    'clickDayname': function(date) { console.log('clickDayname', date); },
    'beforeCreateSchedule': function(e) {
      ...
      if      (e.state === '비공개') data.openType = '1';
      else if (e.state === '공개')   data.openType = '3';
  ...
  'beforeUpdateSchedule': function(e) {
    ...
    if      (e.schedule.state === '비공개') data.openType = '1';
    else if (e.schedule.state === '공개')   data.openType = '3';


    if (e.schedule.raw != null) data.location = e.schedule.raw.location;
    else                        data.location = e.schedule.location;
  ...
  'beforeDeleteSchedule': function(e) {
    ...
    var data = { scheduleId:e.schedule.id }
  ...
  'afterRenderSchedule': function(e) { var schedule = e.schedule; }
});
  ...
  $.ajax({
    ...
    success:function(rs) {
      ...
      $.each(rs.data, function(i, v) {
        var colorIdx = 0;
        $.each(rs['캘린더'], function(ii, vv) { if (vv.commCode === v.scheduleType) colorIdx = ii; });
        ...
        if      (v.openType === '1') scheduleData.state = '비공개';
        else if (v.openType === '3') scheduleData.state = '공개';

        scheduleData.state = '공개';
    ...
    error:function(request,status,error){ reload(); }
  });
}
...
function setEventListener() {
  ...
  $('#dropdownMenu-calendarType').on('click', function(e) {
    return false;
  })
