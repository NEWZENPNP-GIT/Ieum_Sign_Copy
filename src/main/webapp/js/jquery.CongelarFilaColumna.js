		...
		var methods  =
		{
			...
			setup: function ()
			{
				...
				else
				{
					...
					//이미 그려진 테이블이 있다면 다시 추가하지 않는다
					if($fixedColumn.find(".fht-thead").length==0){
						$thead.appendTo($fixedColumn);
					}
					
					var AnchoCol = $thead.find('table').width()+1;

					$thead.find('table').css({'width': AnchoCol,'height':$divHead.outerHeight()});
					$tbody.find('table').css({'width': AnchoCol});
					
					if($fixedColumn.find(".fht-tbody").length != 0 ){
						$fixedColumn.find(".fht-tbody").remove();
					}
		...
		var helpers  =
		{
			...
			*/
			...
			_OnScroll: function($obj)
			{
				var $tabla   = $obj,
					$wrapper = $tabla.closest('.fht-table-wrapper'),
					$thead   = $tabla.siblings('.fht-fixed-body > .fht-thead');
				...
				$tabla.on('scroll', function()
				{
					if( !settings.soloThead )
					{
						...
						$fixedColumns.find('.fht-tbody table').css({
							...
							'margin-top': -$(".fht-fixed-body > .fht-tbody").scrollTop()
						});
					}
